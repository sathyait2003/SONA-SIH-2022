   ...
   include './db_connection.php'
    {
        ...
        // $sql3=mysqli_query($con,"SELECT MAX(feedback_id) as max_id from emp_feedinfo");
        // $row3 = mysqli_fetch_assoc($sql3);
        // $feed_id=$row3['max_id'] + 1;
        $feed_id=123;

        $sql = "INSERT INTO emp_feedinfo values($feed_id,'$college_id','$gen_date','$last_date','$course_id','$batch','$feed_name',$status)";
        $con->query($sql);
        ...
        $retval = mysqli_query($con,"SELECT employer_id,employer_name,employer_email from employer where college_id='$college_id'");    
        if(! $retval ) {
            die('Could not get data: ' . mysqli_error($con));
        }
        //student data retrival and snding mails
        while($row = mysqli_fetch_array($retval,MYSQLI_ASSOC)) {
            $std_id=$row['employer_id'];
            $name=$row['employer_name'];
            $email=$row['employer_email'];
            $url="send_feedback_mailer\employer_dashborad.php?/".$feed_id."/".$std_id;
                            ...
                            <tr style="height: 150px;">
                                <td align="center" style="border: none;
                                    ...
                                    <p style="font-weight: bolder;font-size: 30px;
                                            ...
                                            color:black;">
                                        Dear $name...
                                        <br> kindly fill the employer feedback form.
                                    </p>
                                </td>
                            </tr>
