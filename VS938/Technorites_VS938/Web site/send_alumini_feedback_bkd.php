...
use PHPMailer\PHPMailer\Exception;
    ...
    if(isset($_POST['submit'])){
        ...
        $sql3=mysqli_query($con,"SELECT MAX(feedback_id) as max_id from alumini_feedinfo");
        $row3 = mysqli_fetch_assoc($sql3);
        $feed_id=$row3['max_id'] + 1;

        $sql = "INSERT INTO alumini_feedinfo values($feed_id,'$college_id','$gen_date','$last_date','$course_id','$batch','$feed_name',$status)";
        $con->query($sql);
        ...
        $retval = mysqli_query($con,"SELECT alumini_id,alumini_name,alumini_email from alumini where college_id='$college_id' AND course_id='$course_id' AND batch='$batch'");    
        if(! $retval ) {
            die('Could not get data: ' . mysqli_error($con));
        }
        //student data retrival and snding mails
        while($row = mysqli_fetch_array($retval,MYSQLI_ASSOC)) {
            $alu_id=$row['alumini_id'];
            $name=$row['alumini_name'];
            $email=$row['alumini_email'];
            $url="./alumini_dashboard.php?/".$feed_id."/".$alu_id;
